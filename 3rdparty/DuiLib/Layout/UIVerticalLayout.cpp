...
namespace DuiLib
{
	...
	void CVerticalLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		int iPosY = rc.top;
		if (iChildVAlign == DT_BOTTOM) iPosY = rc.bottom;
		else if (iChildVAlign == DT_VCENTER) iPosY = (rc.top + rc.bottom - cyFixed) / 2;
		if( m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() ) {
		...
		for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
			...
			if (iChildAlign == DT_CENTER) {
				...
				rcCtrl.left = iPosX - rcPadding.right - sz.cx;
				rcCtrl.right = rcCtrl.left + sz.cx;
			}
			else {
				iPosX = rc.left;
			...
			//调整垂直布局
			//
			if (iChildVAlign == DT_TOP || iChildVAlign == DT_VCENTER)
			{

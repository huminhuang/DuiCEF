...
bool CMarkup::LoadFromFile(BYTE **pXmlByte, DWORD &dwSize, LPCTSTR pstrFilename)
{
	CDuiString sFile = CPaintManagerUI::GetResourcePath();
	if (CPaintManagerUI::GetResourceZip().IsEmpty()) {
		sFile += pstrFilename;
		...
		DWORD dwRead = 0;
		*pXmlByte = new BYTE[dwSize];
		::ReadFile(hFile, *pXmlByte, dwSize, &dwRead, NULL);
		::CloseHandle(hFile);
		...
		if (dwSize > 4096 * 1024) return false;
		*pXmlByte = new BYTE[dwSize];
		int res = UnzipItem(hz, i, *pXmlByte, dwSize, 3);
		if (res != 0x00000000 && res != 0x00000600) {
			delete[] * pXmlByte;
			*pXmlByte = NULL;
			if (!CPaintManagerUI::IsCachedResourceZip()) CloseZip(hz);
